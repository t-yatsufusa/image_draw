...
def read_config(path="config.xlsx"):
    ...
    cfg["draw_scale_up"] = float(cfg["draw_scale_up"])
    cfg["draw_scale_down"] = float(cfg["draw_scale_down"])
...
def draw_ip_overlay(img, ip_df, bits, cfg):
    ...
    for _, row in ip_df.iterrows():
        ...
        idx = row["idx"]
        x = int((row["x"] * cfg["mm_to_px_ax"] + cfg["mm_to_px_bx"]) * cfg["draw_scale_up"])
        y = int((row["y"] * cfg["mm_to_px_ay"] + cfg["mm_to_px_by"]) * cfg["draw_scale_up"])
        ...
        cv2.circle(
            img,
            (x, y),
            int(cfg["circle_radius"] * cfg["draw_scale_up"]),
            color,
            -1
        )
...
def main():
    ...
    for i in range(cfg["frame_start"], cfg["frame_end"] + 1):
        ...
        scale_down = cfg["draw_scale_down"]
